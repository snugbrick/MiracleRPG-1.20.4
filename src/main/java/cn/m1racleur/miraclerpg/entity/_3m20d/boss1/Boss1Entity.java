 ...
 * @version 1.0.0 2024.03.18 01:35
 */
public class Boss1Entity extends HostileEntity implements GeoEntity {
    private final AnimatableInstanceCache instanceCache = new SingletonAnimatableInstanceCache(this);

    public Boss1Entity(EntityType<? extends HostileEntity> entityType, World world) {
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if (tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.boss1.walk",
                    Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }
        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.boss1.default_leg_pose",
                Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
